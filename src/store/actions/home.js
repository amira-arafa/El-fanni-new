...
import {
  ...
  STORE_COURSES_LIST,
  STORE_SINGLE_COLLECTION,
  STORE_FAVOURITES_LIST,
} from "../types/home";
...
export const addNewCollection = (name) => async (dispatch) => {
  try {
    await axiosInstance.post(`/collections`, {
      name,
    });
...
export const getMycoursesList = (params) => async (dispatch) => {
  try {
    const res = await axiosInstance.get(`/courses/mycourses`, { params });
    dispatch({
      type: STORE_COURSES_LIST,
      payload: res.data.data,
    });
  } catch (err) {
    console.log(err);
...
export const removeFromCollection =
  (collectionId, courseId) => async (dispatch) => {
    try {
      await axiosInstance.delete(`/collections/remove/${collectionId}`, {
        data: { courseId },
      });
...
export const addToFav = (params) => async (dispatch) => {
  try {
    await axiosInstance.post(`/favorites/${params}`);
    toasters.Success("Added Succesfully");
  } catch (err) {
    console.log(err);
  }
